...
- Compare against the **supplied baseline**, honestly — beating it is not required; a defended
  ...
  unit** and **evaluation mode** with every number. Never report smoke/CI numbers as results. Report the metric **with its spread** across <unit>s (`rep["summary"]`), not a lone pooled number. Grading: [`CAPSTONE_REPORT_RUBRIC.md`](CAPSTONE_REPORT_RUBRIC.md) (team) + [`INDIVIDUAL_ASSESSMENT.md`](INDIVIDUAL_ASSESSMENT.md) (individual).
