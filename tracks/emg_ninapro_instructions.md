...
channel re-ordering. `adapter.denoise()` is importable and gives you the Chapter 8 per-noise-type menu below — but you must call it from a `preprocess()` you write; it is not a `cfg` key on this track.
