# Individual assessment — critical comparison + teamwork (separate from the team's 30 points)

The capstone rubric (`CAPSTONE_REPORT_RUBRIC.md`) grades the **team's** code, report,
and presentation out of 30 points, shared by every member. This document covers the
two things that are graded **per student**, not per team, and are kept as their own
point pools rather than folded into or multiplying the team score — a student's
individual score should be legible on its own, not a mysterious adjustment to a shared
number.

| Component | Points | What it catches |
|---|:-:|---|
| **Critical-comparison essay** | 5 | Whether *this student* personally engaged with a project other than their own, critically — not delegated to whoever on the team felt like writing feedback |
| **Teamwork & contribution** | 5 | Fair workload split, communication, and constructive conflict handling |

---

## Part 1 — Critical-comparison essay (5 points)

Replaces what used to be a closed-book quiz on the student's own track. The reasoning
for the change: a quiz only checks that a student can recall their own track's
concepts — this essay checks that too (you can't write a real comparison without
understanding both projects), *and* it guarantees every student, not just whoever the
team delegates it to, personally engages with a second, different-track project. It
also folds in what used to be a separate team-level **Opposition** criterion in
`CAPSTONE_REPORT_RUBRIC.md` — that feedback is now personal, not a single team
submission.

**Mechanism — shadow pairing:** every team is paired once, early (at track assignment,
before the first project session — see `Course_PM_HT26.md`), with exactly one other
team working a **different track**. Students should look in on their shadow partner's
project informally around the project's mid-point (no deliverable required then — just
enough exposure that the final essay isn't written from a cold read).

**Format:** individual, written, 300–500 words, due shortly after the showcase (once
the student has seen the shadow-paired team's *finished* project, not just their
mid-project state).

**Prompt:** *Compare your own project to your shadow-paired team's project. What's
different in their approach, given a different signal/task? What would you personally
learn or borrow from their choices? What, if anything, would you change about your own
team's approach having seen theirs?*

**What it is not:** generic praise, or a restatement of both projects' results without
comparison. It has to engage with *specific* design choices — a feature choice, a
validation decision, a modelling trade-off — not just "they got a good score."

| Score band | Descriptor |
|---|---|
| 4.5–5 | Specific, technical comparison naming actual design choices in both projects; identifies a concrete, transferable lesson (something the student would genuinely do differently) |
| 3–4 | Real comparison with some specifics, but stays more descriptive ("they did X, we did Y") than critical about what it means |
| 1.5–2.5 | Surface-level — could have been written from the abstract/showcase slide alone, no evidence of engaging with the other project's actual work |
| 0–1 | Generic praise, off-topic, or not submitted |

---

## Part 2 — Teamwork & contribution (5 points)

This is graded from **three sources**, combined by the instructor/TA — never from a
single team member's self-report alone:

1. **A task-assignment log**, submitted by the team early in the capstone period
   (who owns which module — data loading, features, validation, report, etc.) and kept
   updated. This is the reference point for "did you do the share you signed up for,"
   not a retrospective claim. In practice this is the team's **ClickUp workspace** —
   see `../../CM2013 HT26/Project_Management_Guide.md` — whose task history, tags, and
   standup comments serve as this log directly, rather than a separate document.
2. **A private peer-evaluation form**, completed individually by every team member at
   submission, rating every teammate (and briefly themselves) on the three criteria
   below with one sentence of justification per rating. Scores are combined across
   raters (median or a trimmed mean, so one retaliatory or one overly generous rating
   doesn't dominate) and are not shown to teammates in raw form.
3. **A short individual reflection** (150–300 words): *"Describe one disagreement or
   workload imbalance your team encountered, and how it was handled."* If a team
   genuinely had no friction, the student should say so and describe how they kept it
   that way — a well-run team is not penalized for having nothing to report.

### Criteria

| # | Criterion | Points | Full marks | Partial | Minimal | Missing |
|---|---|:---:|---|---|---|---|
| 1 | **Contribution & workload** | 2 | Delivered the module(s) owned in the task log, on schedule, at a quality peers relied on without redoing it | Delivered assigned work, but late or requiring teammates to rework part of it | Contribution was thin relative to the task log, or unclear what this student actually did | Peer consensus + task log indicate the student did not do their assigned share |
| 2 | **Communication & collaboration** | 2 | Proactively communicated progress and blockers, responsive to teammates, attended check-ins | Generally responsive, occasional lapses that didn't derail the team | Inconsistent communication; teammates had to chase for updates more than once | Effectively unreachable or non-participating during team work |
| 3 | **Conflict management** | 1 | Reflection (corroborated by peers) shows a disagreement or imbalance handled constructively — raised directly, discussed, resolved or escalated appropriately | Friction existed and was eventually resolved, but avoided or left unaddressed for a while first | Reflection is vague or generic; unclear whether any real conflict-handling occurred | Reflection indicates conflict was ignored, or peer ratings indicate the student *was* the source of unresolved friction |

**A note on fairness:** peer evaluation is a signal, not a verdict — a single outlier
rating (much lower or higher than the rest) should prompt a look at the task log and
reflection before it moves a grade, not be averaged in blindly. If peer ratings and the
task log clearly disagree with each other, talk to the team before finalizing scores.

**Optional escalation for severe free-riding:** some programs additionally apply a
floor to the *team* score for a student whose peer ratings and task-log evidence show
they did not meaningfully contribute (e.g. capping their share of the team's 30 points
separately from this individual pool). This rubric doesn't build that in by default —
add it only if your course wants that mechanism, since it changes a shared team grade
into a per-student one and should be stated up front if used.
